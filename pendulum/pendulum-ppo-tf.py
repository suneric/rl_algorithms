"""
pendulum PPO with tensorflow implementation
...
https://github.com/nric/ProximalPolicyOptimizationContinuousKeras/blob/master/ppo_keras_continuous.py
"""
...
"""
class ReplayBuffer:
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lamda=0.95):
        self.obs_buf = np.zeros((size, obs_dim), dtype=np.float32) # states
        self.act_buf = np.zeros((size, act_dim), dtype=np.float32) # action, based on stochasitc policy with teh probability
...
class ActorModel:
    ...
    def build_model(self, obsDim, hiddenSize, numActions, lr):
        ...
        for i in range(1, len(hiddenSize)):
            x = tf.keras.layers.Dense(units=hiddenSize[i], activation='tanh')(x)
        mu = tf.keras.layers.Dense(numActions, activation='tanh')(x)
        std = tf.keras.layers.Dense(numActions, activation='softplus')(x)
        mu_std = tf.keras.layers.concatenate([mu,std])
        return tf.keras.Model(inputs = inputs, outputs=mu_std)
    ...
    def predict(self, obs, act=None):
        obs = obs.reshape(1,-1)
        mu_and_std = self.policyNN(obs)
        print(mu_and_std)
        mu = mu_and_std[0]
        std = mu_and_std[1]
        pi = np.random.normal(loc=mu,scale=std,size=self.numActions)
        logprob = None
        if act is not None:
            logprob = tf.math.reduce_sum(pi.log_prob(act), axis=-1)
        return logprob
