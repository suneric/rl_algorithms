...
def encoder(image_shape, latent_dim):
...
def decoder(latent_dim):
    model = keras.Sequential(
        [
            ...
            keras.layers.Conv2DTranspose(filters=1,kernel_size=3,strides=1,padding='same',activation='relu'),
        ]
    )
    print(model.summary())
    return model
...
class VAE(keras.Model):
    def __init__(self, latent_dim):
        ...
        self.encoder = encoder((28,28,1),latent_dim)
    ...
    def encode(self, x):
        ...
        mu, logv = tf.split(logits,num_or_size_splits=2,axis=1)
        return mu, logv
